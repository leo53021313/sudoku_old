...
| 1 | `twist-veteran` | click | 結果殊不知—— | 老油條 GirlVeteran 從右上 spring-in（沿用現有 motion 設定）。標題 clip-path 從左刷出：「以為穩了 · **結果更多關卡等著奶茶**」（紅底反白只在後半段、與現有設計一致）。**奶茶切回 `variant="normal"`**（從喜→預感的過渡）。「告白成功 ✓」貼紙降到 opacity 0.4 + `filter: grayscale(1)`；💗 停止生成（既存的淡出完成後自然消失）。 | — | 1-1.5s 留懸念 |
...
- **奶茶 3-stage mood arc**：原檔無奶茶，新增為主角。`beatIndex >= 0` 入場（中央偏下、spring scale-in）。variant 依 beat 動態：`beatIndex === 0` → `happy`（喜悅版）、`beatIndex === 1` → `normal`（中性，預感未明）、`beatIndex >= 2` → `question`（困惑版）+ 頭頂浮動 ❓（沿用 Ch7Step7 的寫法、`motion.div` 循環上浮 + opacity keyframes、紅字黑描邊）。`MilkTea` 元件的 4-variant prop（normal/happy/crashed/question）在 commit `a5d1279` 已就緒。
