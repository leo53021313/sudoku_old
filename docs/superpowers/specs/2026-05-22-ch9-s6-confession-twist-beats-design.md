...
|---|---|---|---|---|---|---|
| 0 | `confess-success` | click | 最後奶茶終於成功跟對方告白成功—— | 奶茶（**`variant="happy"`**，2026-05-22 新增的心動版）從中央升起 + 黃色「告白成功 ✓」brutalist 貼紙（rotate -8°、box-shadow）。2-3 顆 💗 emoji 從奶茶旁邊浮起淡出（`setInterval` 350ms 生成、最多保留 3 顆、translateY -180px + opacity 0 動畫）。背景乾淨。 | — | 0.8-1.2s 慶祝感 |
| 1 | `twist-veteran` | click | 結果殊不知—— | 老油條 GirlVeteran 從右上 spring-in（沿用現有 motion 設定）。標題 clip-path 從左刷出：「以為穩了 · **結果更多關卡等著奶茶**」（紅底反白只在後半段、與現有設計一致）。**奶茶切回 `variant="normal"`**（從喜→預感的過渡）。**「告白成功 ✓」貼紙完全退場**（scale 0 + opacity 0，0.4s ease overshoot）；💗 停止生成（既存的淡出完成後自然消失）。 | — | 1-1.5s 留懸念 |
| 2 | `traps-rain` | click | 前面還有更多關卡等著奶茶 | 4 張陷阱題卡延用現有 cascade（rotate spring + boxShadow，每張 0.15s stagger、overshoot ease `[0.34, 1.56, 0.64, 1]`）。**中央奶茶 fade-out**（scale 0 + opacity 0，0.3s）；**左下角 reaction-shot 奶茶 fade-in**（width 130、`variant="question"`、scale 0.5→1 + opacity 0→1、0.5s delay 0.2s）+ 頭頂 1-2 顆浮動 ❓（沿用 Ch7Step7 beat 6 的 `?` 浮動寫法）。觸發 `triggerShake()` 一次（climax B）。**舞台變三分法構圖：老油條右上 / 4 卡中央 / 奶茶左下。** | `['B']` | 2-3s 觀眾消化、接 s7 |
...
- **奶茶 3-stage mood arc + 雙位置 layout**：原檔無奶茶，新增為主角。**兩個 MilkTea 實體 crossfade**：
  - **中央 hero 奶茶**（beat 0/1 可見、beat>=2 fade-out 退場）：中央偏下 spring-in、`variant={beatIndex >= 1 ? 'normal' : 'happy'}`、`beatIndex >= 2` 時 `scale: 0 + opacity: 0`（0.3s 退場）。
  - **左下角 reaction-shot 奶茶**（beat 2 fade-in）：width 130（縮小）、`variant="question"` 固定、`beatIndex >= 2` 時 `scale: 0.5→1 + opacity: 0→1`（0.5s delay 0.2s，比中央退場慢一點，crossfade 不重疊太久）+ 頭頂 1-2 顆浮動 ❓（沿用 Ch7Step7 的寫法、`motion.div` 循環上浮 + opacity keyframes、紅字黑描邊）。
  - **為什麼用雙實體 crossfade 而非單實體 motion 平移**：motion/react 動畫 `left` 在 % ↔ px 之間插值會 jiggle，動畫 `x` 又需要 viewport math。雙實體 + 錯時 crossfade（中央先淡出、角落後淡入）視覺結果是「奶茶從中央被擠到角落」的故事感，技術上簡單可靠。
  - `MilkTea` 元件的 4-variant prop（normal/happy/crashed/question）在 commit `a5d1279` 已就緒。
