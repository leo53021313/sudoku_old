...
class PhaseManager:
    ...
    def record_episode(self, success: bool, mrv_step: int) -> bool:  # True = phase just advanced
        with self._lock:
    ...
    def mrv_prob(self, mrv_step: int) -> float:  # piecewise cosine MRV probability
        cfg = self._cfg
        with self._lock:
            phase = self._phase
    ...
    def bc_exponent(self) -> float:  # 0.5 / 1.0 / 999.0 per phase
        with self._lock:
            return {
                self.PHASE_1: 0.5,
                self.PHASE_2: 1.0,
