...
class DashboardWindow(QMainWindow):
    ...
    def _dispatch(self, ev) -> None:
        from app.gui.event_bus import (
            EpisodeStartEvent, BoardUpdateEvent, EpisodeEndEvent,
            StatsUpdateEvent, PoolUpdateEvent, ProxyUpdateEvent,
            ProducerUpdateEvent, StateChangeEvent, ModelSavedEvent,
        )
        # Typed event path (new API)
        if isinstance(ev, EpisodeStartEvent):
            self._on_episode_start({
                "board": ev.board, "fixed": ev.fixed,
                "episode_idx": ev.episode_idx, "level": ev.level,
            })
            return
        if isinstance(ev, BoardUpdateEvent):
            self._on_board_update({
                "board": ev.board, "fixed": ev.fixed,
                "highlight": ev.highlight, "episode_idx": ev.episode_idx,
            })
            return
        if isinstance(ev, EpisodeEndEvent):
            self._on_episode_end({
                "board": ev.board, "fixed": ev.fixed,
                "success": ev.success, "episode_idx": ev.episode_idx,
                "total_reward": ev.total_reward, "level": ev.level,
            })
            return
        if isinstance(ev, StatsUpdateEvent):
            self._on_stats_update({
                "episode_idx": ev.episode_idx, "total_episodes": ev.total_episodes,
                "update_count": ev.update_count, "mrv_prob": ev.mrv_prob,
                "entropy": ev.entropy, "loss": ev.loss,
                "rollout_size": ev.rollout_size, "rollout_cap": ev.rollout_cap,
                "phase": ev.phase,
            })
            return
        if isinstance(ev, PoolUpdateEvent):
            self._on_pool_update({"unsolved": ev.unsolved, "total": ev.total})
            return
        if isinstance(ev, ProxyUpdateEvent):
            self._proxy_valid = ev.valid
            self._proxy_total = ev.total
            self._refresh_pool_ui()
            return
        if isinstance(ev, ProducerUpdateEvent):
            self._prod_ok      += ev.success_delta
            self._prod_fail    += ev.fail_delta
            self._prod_blocked += ev.blocked_delta
            return
        if isinstance(ev, StateChangeEvent):
            self._apply_state(ev.state)
            return
        if isinstance(ev, ModelSavedEvent):
            self._info_bar.set_log(f"[已儲存] Ep {ev.episode_idx} → {ev.path}")
            return
    ...
    def _on_episode_start(self, d: dict) -> None:
        board   = d.get("board", [[0]*9 for _ in range(9)])
        fixed   = d.get("fixed", [[False]*9 for _ in range(9)])
